...
DEFAULT_PASSWORD="firefly"  #SCNKZwnv9pbFd123
...
ssh_cmd() {
    # 优先使用 SSH 密钥，如果有 sshpass 则使用密码
    if command -v sshpass &> /dev/null && [ -n "$TARGET_PASSWORD" ]; then
        ...
        ssh -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -o LogLevel=ERROR -o ConnectTimeout=10 "$TARGET_USER@$TARGET_HOST" "$@"
    fi
}
...
do_deploy() {
    ...
    # 使用 scp 上传（优先使用 SSH 密钥，如果有 sshpass 则使用密码）
    if command -v sshpass &> /dev/null && [ -n "$TARGET_PASSWORD" ]; then
        ...
        scp -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null "$PACKAGE_FILE" "$TARGET_USER@$TARGET_HOST:$REMOTE_FILE"
    fi
