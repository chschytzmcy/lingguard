...
LingGuard 的 Heartbeat 采用**纯轮询检查**模式，不引入复杂的事件驱动机制。这是经过深思熟虑的设计决策。

##### 核心区别：Heartbeat vs Cron

| 维度 | Heartbeat | Cron（定时任务） |
|------|-----------|-----------------|
| **本质** | **轮询检查** + 智能过滤 | **精确触发** + 执行任务 |
| **时间精度** | 大约每30分钟（可延迟） | 精确到秒 |
| **触发方式** | 定时器 → 问Agent"有什么要处理的吗？" | 定时器 → 执行具体任务 |
| **上下文** | 主会话上下文（有记忆） | 可隔离（无历史） |
| **输出** | 只有"有事情"才通知 | 执行完即输出 |

用一个类比：

```
Heartbeat 像一个"智能管家"，每隔一段时间走过来问：
"有什么需要我处理的吗？"
- 他知道你最近的对话、正在做的事情
- 他会自己判断是否需要打扰你
- 如果一切正常，他只说"HEARTBEAT_OK"然后走开
- 多个检查合并成一次询问
...
OpenClaw 采用了更复杂的**事件驱动 Heartbeat**：

```
OpenClaw 的设计（我们不采用）：
┌─────────────────────────────────────────────────────────────┐
│ Exec 完成事件 ──→ 产生系统事件 ──→ 触发 Heartbeat           │
│ Cron 到期事件 ──→ 产生系统事件 ──→ 触发 Heartbeat           │
│ 通知变化事件 ──→ 产生系统事件 ──→ 触发 Heartbeat           │
└─────────────────────────────────────────────────────────────┘
```

**LingGuard 选择不采用这种设计的原因：**

1. **实时交互场景不需要**
   - 用户在会话中提交任务，结果直接在当前会话返回
   - 不需要通过 Heartbeat 做中转通知

2. **避免不必要的复杂度**
   - OpenClaw 是企业级多渠道 Gateway，需要处理：
     - 多种后台进程
     - 多 Agent 实例
     - 复杂的会话管理
   - LingGuard 主要是实时对话交互，保持简单即可

3. **精确时间需求用 Cron 解决**
   - 如果需要精确时间触发，直接用 Cron
   - 不需要"事件 → Heartbeat → 通知"这种延迟批处理

##### 设计决策总结

| 功能 | LingGuard 是否需要 | 原因 |
|------|-------------------|------|
| Heartbeat 轮询检查 | ✅ 已有 | 定期检查任务，智能过滤通知 |
| 精确时间触发 | ✅ 用 Cron | 直接调度，不绕道 Heartbeat |
| Exec→Heartbeat 事件 | ❌ 不需要 | 实时交互场景，结果直接返回 |
| Cron→Heartbeat 事件 | ❌ 不需要 | 用 Cron 直接执行更精确 |
...
| 任务类型 | 特征 | 推荐方案 |
|---------|------|---------|
| **固定周期性行为** | 明确、稳定、长期存在 | Heartbeat |
| **单次/临时通知** | 一次性、动态创建 | Cron `--at` |
| **变化较快的周期任务** | 频繁调整、不确定周期 | Cron |

**适合 Heartbeat 的场景（固定周期性行为）：**
- 每天检查日程（固定行为，长期存在）
- 定期检查系统状态（固定行为，长期存在）
- 每日记忆提炼（固定时间，固定流程）
- 社交互动检查（如 Moltbook 社区消息）
...
```bash
# Cron - 单次/临时通知（动态创建）
lingguard cron add --name "提醒" --at "20m" --message "该打电话了" --delete-after-run
lingguard cron add --name "会议" --at "09:00" --message "10分钟后开会" --delete-after-run
```

**错误用法：**
```
❌ 用 Heartbeat 做单次提醒（应该用 Cron --at）
❌ 用 Heartbeat 做频繁变化的任务（应该用 Cron）
❌ 用 Cron 做固定检查项（应该合并到 HEARTBEAT.md）
❌ 会话内任务结果通过 Heartbeat 通知（应该直接返回）
```
