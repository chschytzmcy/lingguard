---
name: meos-file
description: MEOS/MeBox 文件管理 HTTP API skill - 提供远程设备文件管理接口，包括创建、删除、元数据、标签、搜索、版本控制、上传、下载等操作；触发关键字：MEOS、meos、密盒、小密盒、会议助手、小密盒智会通、MEBOX、mebox、MeBox、盒子文件、盒子搜索、上传文件、下载文件、文件上传、文件下载
---

# MEOS 文件服务 API

本 skill 提供 MEOS 文件服务 (`drive.object.v1.ObjectService`) 的完整 HTTP API 规范。
...
|------|------|------|------|
| name | 文件名 | string | |
| file_type | 文件类型 | enum, array[enum] | 1=文件夹，2=普通文件，3=链接文件, 4=标签文件 |
| content_type | 文件内容类型 | enum, array[enum] | 1=image, 2=video, 3=audio, 4=document, 5=misc |
| sub_type | 文件子类型 | string, array[string] | image/png, application/pdf |
| category | 分类标签 | string, array[string] | 音乐、影视、笔记、书籍 |
| upload_time | 上传时间 | time range | {"start_time": 1688994978, "end_time": 1693968135} |
| create_time | 创建时间 | time range | {"start_time": 1688994978, "end_time": 1693968135} |
| upload_uid | 上传者uid | string | |
| from_uid | 来源用户 | string | |
| from_device_id | 设备ID | string | 20e5a161-23f8-4d6c-b0f6-fdc22cc183c4 |
| from_app | 来源应用 | string | appId/instanceId |
| sc | 星级/存储策略 | int | 1=sc1x, 2=sc2x |
| desc_tag | 描述标签 | array[string] | ["艺人:周杰伦", "艺人:张学友"] |
| user_tag | 用户自定义标签 | array[string] | ["技术", "重要"] |

#### 检索语法

**组合条件检索 (query)**
- query 参数是 Term 查询条件数组
- Term 之间是**与(AND)**关系
- Term 内是**或(OR)**关系
...
**2. 关键字检索（自动映射 name, desc_tag, user_tag）：**
```json
{
  "key": "关键词"
}
...
```
注意 ⚠️：
由于下载逻辑是基于重定向下载，所以必须传递 --location 参数
```
